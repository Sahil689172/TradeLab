...
def write_charts(result: WalkForwardResult, *, output_dir: Path) -> dict[str, Path]:
    ...
    if len(equity):
        ...
        ax.set_title("Combined OOS equity")
    ...
    if result.windows:
        ...
        ax.bar([i + 0.2 for i in range(len(labels))], [w.oos.sharpe for w in result.windows], width=0.4, label="OOS")
        ax.set_xticks(range(len(labels)))
        ax.set_xticklabels(labels, rotation=45, ha="right")
