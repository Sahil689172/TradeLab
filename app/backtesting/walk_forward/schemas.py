...
class DegradationReport(BaseModel):
    ...
    note: str = (
        "Degradation is descriptive. It is not an automatic fail. "
        "Ratios are OOS / Train. Degradation % is (Train − OOS) / |Train|."
    )
...
class ParameterStability(BaseModel):
    ...
    note: str = (
        "Stability score is 1 − changes/(windows−1). Fragile tuning is a diagnostic, "
