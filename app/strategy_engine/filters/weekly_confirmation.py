"""Weekly timeframe confirmation filter."""
...
class WeeklyConfirmationFilterConfig(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")

    trend_keys: tuple[str, ...] = ("weekly_trend", "weekly_trend_direction", "w1_trend")
    require_bullish_for_buy: bool = True
    require_bearish_for_sell: bool = True
    only_when_requested: bool = False


class WeeklyConfirmationFilter(FilterBase):
    """Confirm signals against the weekly trend regime."""

    confirmation_id = WEEKLY

    def __init__(
        self,
        *,
        name: str = "weekly_confirmation",
        enabled: bool = True,
        priority: int = 82,
        config: WeeklyConfirmationFilterConfig | None = None,
        **threshold_overrides: object,
    ) -> None:
        super().__init__(name=name, enabled=enabled, priority=priority)
        base = config or WeeklyConfirmationFilterConfig()
        self._config = base.model_copy(update=threshold_overrides) if threshold_overrides else base

    @property
    def config(self) -> WeeklyConfirmationFilterConfig:
        return self._config
    ...
    def validate(self, recommendation: StrategyRecommendation) -> None:
        ...
        if (
            ...
            raise FilterValidationError(
                f"{self.name}: BUY requires weekly BULLISH (got {trend})",
            )
        if (
            ...
            raise FilterValidationError(
                f"{self.name}: SELL requires weekly BEARISH (got {trend})",
            )
    ...
    def apply(self, recommendation: StrategyRecommendation) -> StrategyRecommendation:
        ...
        return annotate(
            recommendation,
            note=f"{self.name}: pass weekly_trend={trend}",
            updates={"filter_weekly_trend": trend},
        )
